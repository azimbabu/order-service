...
class OrderServiceApplicationTests {
  ...
  static KeycloakContainer keycloakContainer = new KeycloakContainer(
      "quay.io/keycloak/keycloak:19.0").withRealmImportFile("test-realm-config.json");
  ...
  void whenGetOrdersForAnotherUserThenNotReturned() throws IOException {
    ...
    given(bookClient.getBookByIsbn(bookIsbn)).willReturn(Mono.just(book));
    OrderRequest orderRequest = new OrderRequest(bookIsbn, 1);
    Order orderByBjorn = webTestClient
        .post()
        .uri("/orders")
        .headers(headers -> headers.setBearerAuth(bjornTokens.accessToken()))
        .bodyValue(orderRequest)
    ...
    Order orderByIsabelle = webTestClient
        .post()
        .uri("/orders")
    ...
    webTestClient
        ...
        .value(orders -> {
          assertThat(orders.stream().filter(order -> order.bookIsbn().equals(bookIsbn))
              .findAny()).isNotEmpty();
          Set<Long> orderIds = orders.stream().map(Order::id).collect(Collectors.toSet());
