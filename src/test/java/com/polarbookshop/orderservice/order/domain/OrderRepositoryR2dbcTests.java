...
class OrderRepositoryR2dbcTests {
    ...
    void whenCreateOrderNotAuthenticatedThenNoAuditMetadata() {
        ...
        StepVerifier.create(orderRepository.save(rejectedOrder))
            .expectNextMatches(
                order -> order.status().equals(OrderStatus.REJECTED) && Objects.isNull(
                    order.createdBy()) &&
    ...
    void whenCreateOrderAuthenticatedThenAuditMetadata() {
        var rejectedOrder = OrderService.buildRejectedOrder("1234567890", 3);
        String username = "marlena";
        StepVerifier.create(orderRepository.save(rejectedOrder))
            .expectNextMatches(
                order -> order.status().equals(OrderStatus.REJECTED) && order.createdBy().equals(
                    username) &&
                    order.lastModifiedBy().equals(username))
            .verifyComplete();
    }
